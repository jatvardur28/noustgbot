...
MEMORY_SETTINGS = {
    "max_messages_per_user": 20,  # Максимум сообщений в памяти
    "context_cleanup_hours": 2,   # Очистка старых контекстов через 2 часа
...
async def cmd_start(message: Message):
    ...
    welcome_text = f"""
🤖 **Привет, {user_name}!** Добро пожаловать в AI-бота с памятью!
...
• 🚀 **Hermes 405B** - Максимально мощная для сложных задач
...
Теперь для модели DeepHermes 24B будут показываться теги `<think>` с процессом размышления модели.

💡 Это поможет понять, как модель анализирует ваши запросы и приходит к ответам.

⚠️ На модель Hermes 405B режим отладки не влияет.
        """
    else:
        text = """
...
🆕 **Память и контекст:**
• Бот запоминает до 20 последних сообщений
• Может ссылаться на предыдущие части разговора
• Память очищается через 2 часа бездействия
...
• **Hermes 405B** - Максимальная мощность
